...
@pytest.mark.parametrize("shuffle_ids", [False, True])
def test_data_file_topology(snap_top, tmp_path, shuffle_ids):
    ...
    snap_top.bonds.id = bond_id
    snap_top.bonds.typeid = [1, 2, 1, 2, 1, 2]
    ...
    snap_top.angles.id = angle_id
    snap_top.angles.typeid = [1, 2, 2, 1]
    ...
    snap_top.dihedrals.id = dihedral_id
    ...
    snap_top.impropers.id = improper_id
    snap_top.impropers.typeid = [1, 2]
    snap_top.impropers.members = [
        [1, 2, 3, 4],
        [5, 6, 7, 8],
    ]
    ...
    # test impropers
    assert snap_2.impropers.N == snap_top.impropers.N
    assert snap_2.impropers.has_id()
    assert numpy.allclose(snap_2.impropers.id, snap_top.impropers.id)
    assert snap_2.impropers.has_typeid()
    assert numpy.allclose(snap_2.impropers.typeid, snap_top.impropers.typeid)
    assert snap_2.impropers.has_members()
    assert numpy.allclose(snap_2.impropers.members, snap_top.impropers.members)
